...
function MessagePage() {
  ...
  const setSelectedProfile = useMessageStore(
  ...
  const { _id: userId, accountType } = useUserStore((state) => state.user);
  ...
  const handleMessageFilter = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      const filtered = Object.entries(messages).reduce(
        (acc, [senderId, messageGroup]) => {
          const filteredMessagesForSender = messageGroup.messages.filter(
            (message) => message.senderName.toLowerCase().includes(searchValue)
          );
          ...
          <div className="container mx-4 d-flex tw-gap-3 tw-max-h-[500px]">
            <div className="tw-flex tw-flex-col tw-bg-white tw-border tw-rounded-md tw-p-2">
              ...
              <div className="tw-gap-2 tw-flex tw-flex-col tw-overflow-auto">
                {Object.keys(filteredMessages)?.map(
                  (senderId: string, index: number) => {
                    const senderName =
                      filteredMessages[senderId]?.senderName || "";
                    const message =
                      filteredMessages[senderId]?.messages?.[0]?.message || "";
