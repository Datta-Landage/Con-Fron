                ...
                <div className='sign-box bg-warning d-flex justify-content-between align-items-center py-4 rounded'>
                    ...
                    <div className='form rounded'>
                        <div className='Signing-optn d-flex justify-content-between pb-4'>
                            <Link to='/signup' className='text-decoration-none '><h5>Sign Up</h5></Link>
                            <Link to='/signin' className='text-decoration-none text-secondary'><h4>Sign In</h4></Link>
                        </div>
                        <div>
                            <form>
                                ...
                                <div className={` my-2 ${error && !validateEmail(email) ? 'error-border' : 'inputbox'}`}>
                                    <label className='cid-label'>Email</label><br />
                                    <input type="email" className="form-control"
                                    value={email} 
                                    onChange={(e) => { setEmail(e.target.value) }} 
                                    name="email" placeholder="Email Id" required />
                                    {error && !validateEmail(email) && (<span className="error-text text-danger"><small>Invalid email</small></span>)}
                                </div>
                                <div className={` my-2 ${error && !validatePassword(password) ? 'error-border' : 'inputbox'}`}>
                                    <label className='cid-label'>Password</label><br />
                                ...
                                <div className={`my-2 ${error && !validatePassword(repassword) ? 'error-border' : 'inputbox'}`}>
                                    <label className='cid-label'>Re-enter Password</label><br />
                                    <input type='password' className='form-control'
                                    value={repassword} 
                                    onChange={(e) => { setRepassword(e.target.value) }} name='repassword' required />
                                    {error && !validatePassword(repassword) && (<span className="error-text text-danger"><small>It must match with you password</small></span>)}
                                </div>
                                <button id='sign' className="btn-primary-cidblue p-2 my-3 w-100" onClick={saveUser}>Sign Up</button>
                            </form>
